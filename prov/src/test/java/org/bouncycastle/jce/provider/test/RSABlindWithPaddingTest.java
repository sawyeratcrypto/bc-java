...
import org.bouncycastle.crypto.engines.RSABlindEncryptionEngine;
...
public class RSABlindWithPaddingTest extends SimpleTest {
    ...
    public void performTest() throws Exception {
        ...
        RSAKeyParameters privateKeyParameters = new RSAKeyParameters(true, privKeySpec.getModulus(), privKeySpec.getPrivateExponent() );

        // encrypt
        Cipher enCipher = Cipher.getInstance("RSA/ECB/PKCS1Padding", "BC");
        ...
        // un padding
        RSABlindEncryptionEngine rsaBlindEncryptionEngine =  new RSABlindEncryptionEngine();
        rsaBlindEncryptionEngine.init(false, privateKeyParameters);

        PKCS1Encoding pkcs1Encoding = new PKCS1Encoding(rsaBlindEncryptionEngine);
        // supply private to trigger un padding
        pkcs1Encoding.init(false, privateKeyParameters);

        byte[] unPadding =  pkcs1Encoding.processBlock(unBlind,0, message.length());
        System.out.println("unPadding: " + Arrays.toString(unPadding));
        System.out.println("message: " + Arrays.toString(plaintext));

        if (!areEqual(unPadding, plaintext))
        {
            ...
            String[]    args)
    {
        ...
        runTest(new RSABlindWithPaddingTest());
    }
}
